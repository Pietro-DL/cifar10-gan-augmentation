...
📁 github/
├── 📁 images (.pt files of the generated datasets, ignored by git)
├── 📁 model_weights (.pth files ignored by git)
...
# ⚠️ Important Notice Regarding Repository Contents
Please note that the heavy network parameters and dataset tensors are not included in this remote repository. Due to GitHub's strict file-size limitations for standard uploads, the entire model_weights directory—which contains the .pth files like augmented_cifar_cnn.pth and cWGAN_GP_netG.pth—has been explicitly ignored by Git.  Likewise, the large synthetic .pt dataset tensors generated by the GANs are excluded. The scripts provided in this repository are fully modular and configured to regenerate all datasets and model weights from scratch upon execution. If required for direct evaluation, the pre-trained weights and synthetic datasets can be provided separately via a direct cloud storage link.
...
This script executes a comprehensive evaluation of all ablation models, generating detailed metrics and visualizations to analyze the impact of varying data ratios.

*   **1. Setup & Dynamic Pathing:** 
    *   Utilizes PyTorch and Python's `os` module to dynamically resolve the repository directory, ensuring cross-platform execution.
*   **2. Defining the Classifier Architecture:** 
    *   Reinstantiates the architectural blueprint of the VGG-style CNN to provide the necessary spatial mapping for PyTorch to successfully load the pre-trained `.pth` weights into memory.
*   **3. Preparing the Strict Test Set:** 
    *   Loads the CIFAR-10 test set with a strict exclusion of geometric augmentations, providing a mathematically sterile baseline for evaluation.
